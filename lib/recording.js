...
module.exports = function (Cam) {
		...
		this._request(
			...
			function (err, data, xml) {
				if (!err) {
					 ...
					 */
					this.recordingItems =
						linerase(data).getRecordingsResponse.recordingItem;
				}
				if (callback) {
		...
		this._request(
			...
			function (err, data, xml) {
				if (!err) {
					 ...
					 */
					this.jobItem =
						linerase(data).getRecordingJobsResponse.jobItem;
				}
				if (callback) {
	...
	Cam.prototype.createRecordingJob = function (options, callback) {
		...
		try {
			commandSend =
				'<CreateRecordingJob xmlns="http://www.onvif.org/ver10/recording/wsdl">' +
				'<JobConfiguration>' +
				'<ScheduleToken>' +
				(options.scheduleToken || '') +
				'</ScheduleToken>' +
				'<RecordingToken>' +
				(options.recordingToken || '') +
				'</RecordingToken>' +
				'<Mode>' +
				(options.mode || '') +
				'</Mode>' +
				...
				'<SourceToken>' +
				'<Type>' +
				(options.source.sourceToken.type || '') +
				'</Type>' +
				'<Token>' +
				(options.source.sourceToken.token || '') +
				'</Token>' +
				'</SourceToken>' +
				'<AutoCreateReceiver>' +
				(options.source.autoCreateReceiver || '') +
				'</AutoCreateReceiver>' +
				'<Tracks>' +
				'<SourceTag>' +
				((((options ? options.source : '') ? options.source.tracks : '')
					? options.source.tracks.sourceTag
					: '') || '') +
				'</SourceTag>' +
				'<Destination>' +
				((((options ? options.source : '') ? options.source.tracks : '')
					? options.source.tracks.destination
					: '') || '') +
				'</Destination>' +
				'</Tracks>' +
				'<Extension>' +
		...
		this._request(
			...
			function (err, data, xml) {
				if (!err) {
					this.recordingJob =
						linerase(data).createRecordingJobResponse;
				}
				if (callback) {
			...
			function (err, data, xml) {
				...
				if (!err) {
					getRecordingJobsResponse =
						linerase(data).getRecordingJobsResponse;
				}
				if (callback) {
			...
			function (err, data, xml) {
				if (!err) {
					this.recordingSummary =
						linerase(data).getRecordingSummaryResponse.summary;
				}
				if (callback) {
	...
	Cam.prototype.getRecordingJobState = function (options, callback) {
		this._request(
			...
			function (err, data, xml) {
				if (!err) {
					this.recordingJobState =
						linerase(data).getRecordingJobStateResponse.state;
				}
				if (callback) {
			...
			function (err, data, xml) {
				...
				if (!err) {
					deleteRecordingJobResponse =
						linerase(data).deleteRecordingJobResponse;
				}
				if (callback) {
			...
			function (err, data, xml) {
				if (!err) {
					this.recordingOptions =
						linerase(data).getRecordingOptionsResponse.options;
				}
				if (callback) {
	...
	Cam.prototype.getRecordingJobConfiguration = function (options, callback) {
		this._request(
			...
			function (err, data, xml) {
				if (!err) {
					this.recordingJobConfiguration =
						linerase(
							data,
						).getRecordingJobConfigurationResponse.jobConfiguration;
				}
				if (callback) {
					callback.call(
						this,
